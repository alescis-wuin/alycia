...
### Lot 10.7 generation observability invariants

Tests must prove that llama.cpp generation requests opt into the provider's final streamed usage chunk without adding generation retries; input/output/total/cached token counts and optional timing/rate metadata are parsed independently from text/reasoning deltas; completed, caller-cancelled, and failed generations produce distinct provider-neutral observations; only failed observations carry a safe failure kind; end-to-end and first-output latency remain non-negative; the latest observation is refreshed into Presentation after generation ends; and the structured local observation log contains provider/model/version/metrics but never the request prompt/messages, reasoning, response content, raw HTTP bodies, API keys, endpoints, log tails, or exception diagnostics.
