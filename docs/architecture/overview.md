...
The desktop executable composition root for Windows, Linux, and macOS. It selects the local JSON conversation runtime, chooses platform-local data directories, creates the managed `LlamaCppProviderRuntime`, registers it under a stable provider identifier, creates the global JSON configuration store with Lot 08 legacy-model migration, wires the registry into `StreamConversationTurnUseCase`, injects registry/configuration ports into Presentation, disposes the managed server at shutdown, and starts Avalonia. Other hosts can make different infrastructure choices while reusing `Alicia.Presentation`.
