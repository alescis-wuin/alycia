...
- The managed llama.cpp server binds to loopback only, selects a per-launch available port, restricts CORS to localhost, and disables the bundled Web UI.
- Each managed llama.cpp process receives a cryptographically random ephemeral API key through `LLAMA_API_KEY`; Alicia sends it only as a local Bearer credential, validates ownership through a protected `/props` handshake, and never stores the secret in Git, provider configuration, conversation data, command-line arguments, or logs.
